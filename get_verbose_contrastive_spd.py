...
# --------- FIXED FUNCTION (shell sampling) ----------
def random_centroids_around(cz, cy, cx, sz, sy, sx, shape, n=5,
                            min_radius_ratio=0.0, max_radius_ratio=0.3):

    R = max(sz, sy, sx)
    min_r = min_radius_ratio * R
    max_r = max_radius_ratio * R
    ...
    for _ in range(n):
        ...
        dz = int(r * np.cos(theta) * np.sin(phi))
        dy = int(r * np.sin(theta) * np.sin(phi))
        dx = int(r * np.cos(phi))

        z = np.clip(cz + dz, 0, shape[0]-1)
        y = np.clip(cy + dy, 0, shape[1]-1)
        x = np.clip(cx + dx, 0, shape[2]-1)

        centroids.append((z, y, x))

    return centroids

def extract_spd_from_centroid(t2, adc, bval, cz, cy, cx, sz, sy, sx):
...
SAVE_PATH = '/Datasets/verbose_malignant_spd_picai_contrastive'
...
for patient_id in tqdm(info, desc="Processing patients", unit="vol"):
    ...
    # Main
    spd_main = extract_spd_from_centroid(t2, adc, bval, cz, cy, cx, sz, sy, sx)
    np.save(os.path.join(SAVE_PATH, f"{patient_id}_cov_0.npy"), spd_main)

    # Positives
    if info[patient_id]["case_csPCa"] == "YES":

        # Inner shell
        rand_centroids_inner = random_centroids_around(
            cz, cy, cx, sz, sy, sx, t2.shape,
            n=5, min_radius_ratio=0.0, max_radius_ratio=0.3
        )

        for i, (rz, ry, rx) in enumerate(rand_centroids_inner, start=1):
            spd_rand = extract_spd_from_centroid(t2, adc, bval, rz, ry, rx, sz, sy, sx)
            np.save(os.path.join(SAVE_PATH, f"{patient_id}_cov_positive_inner{i}.npy"), spd_rand)

        # Outer shell (true outer context)
        rand_centroids_outer = random_centroids_around(
            cz, cy, cx, sz, sy, sx, t2.shape,
            n=5, min_radius_ratio=3.0, max_radius_ratio=4.0
        )

        for i, (rz, ry, rx) in enumerate(rand_centroids_outer, start=1):
            spd_rand = extract_spd_from_centroid(t2, adc, bval, rz, ry, rx, sz, sy, sx)
            np.save(os.path.join(SAVE_PATH, f"{patient_id}_cov_positive_outer{i}.npy"), spd_rand)
