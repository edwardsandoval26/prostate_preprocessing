...
def random_centroids_around(cz, cy, cx, sz, sy, sx, shape, n=5, radius_ratio=0.3):
    max_r = radius_ratio * max(sz, sy, sx)
    centroids = []
...
SAVE_PATH = '/Datasets/verbose_malignant_spd_picai'
...
for patient_id in tqdm(info, desc="Processing patients", unit="vol"):
    ...
    if info[patient_id]["case_csPCa"] == "YES":
        rand_centroids = random_centroids_around(
            cz, cy, cx, sz, sy, sx, t2.shape, n=5, radius_ratio=0.3
        )

        for i, (rz, ry, rx) in enumerate(rand_centroids, start=1):
            spd_rand = extract_spd_from_centroid(t2, adc, bval, rz, ry, rx, sz, sy, sx)
            np.save(os.path.join(SAVE_PATH, f"{patient_id}_cov_rand{i}.npy"), spd_rand)
